...
export function rateLimitHit(ip, { max = 60, windowMs = 60_000, scope = "default" } = {}) {
  const now = Date.now();
  // Keep limits independent: a chat-save or PDF poll must not consume the
  // budget for artifact generation just because they share an IP address.
  const key = `${String(ip ?? "unknown")}:${scope}:${max}:${windowMs}`;
